---
title: "Joint likelihood using fully and partially annotated trees"
author: "George G Vega Yon"
date: "Sep 26, 2019"
output:
  beamer_presentation:
    keep_tex: true
header-includes:
  - \usepackage{booktabs}
  - \usepackage{tabularx}
  - \usepackage{geometry}
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, fig.height=7)
library(aphylo)
library(xtable)
```

```{r read-data}
files <- list.files(pattern = "^(mcmc|mle).+\\.rds", full.names = FALSE)
estimates <- lapply(files, readRDS)

# Creating a copy, we need the full chains for later
estimates0 <- estimates

# Splitting chains
for (i in which(grepl("^mcmc", sapply(estimates, "[[", "method")))) {
  estimates[[i]] <- window(
    x     = estimates[[i]],
    start = (end(estimates[[i]]$hist) - start(estimates[[i]]$hist))/2
  )
}

```

```{r computing-tables, results='asis', cache = TRUE, dependson=-1}
theta <- do.call(cbind, lapply(estimates, coef))
vars  <- do.call(cbind, lapply(lapply(estimates, vcov), diag))

tab  <- matrix(sprintf("%.2f", theta), ncol = ncol(theta))
dimnames(tab) <- list(rownames(theta), paste0("(", 1:ncol(tab), ")"))

varnames <- c(
  psi0  = "$\\psi_{01}$",
  psi1  = "$\\psi_{10}$",
  mu_d0 = "$\\mu_{d01}$",
  mu_d1 = "$\\mu_{d10}$",
  mu_s0 = "$\\mu_{s01}$",
  mu_s1 = "$\\mu_{s10}$",
  Pi    = "$\\pi$"
  )

rownames(tab) <- varnames[rownames(tab)]

# Adding information

ps <- lapply(estimates, prediction_score, loo=TRUE)
aucs <- sapply(ps, function(i) {
  mean(sapply(i, function(j) j$auc$auc))
})
pss  <- sapply(ps, function(i) {
  tmp <- t(sapply(i, function(j) with(j, c(obs, random))))
  colMeans(tmp, na.rm = TRUE)
})

tab <- rbind(
  tab,
  `Tree count` = sapply(estimates, Ntrees),
  Method       = ifelse(grepl("mcmc", files), "MCMC", "MLE"),
  Prior        = ifelse(grepl("no_prior", files), "Uniform", "Beta"),
  Inferred     = ifelse(grepl("fully", files), "Yes", "No"),
  AUC          = sprintf("%.2f", aucs),
  `P. Score (obs)` = sprintf("%.2f", pss[1,]),
  `P. Score (random)` = sprintf("%.2f", pss[2,])
  )
```

```{r writing-tables}
# Splitting by type
tab_inferred <- tab[, tab["Inferred",] == "Yes"]
tab_observed <- tab[, tab["Inferred",] == "No"]

tables <- list()
for (tab. in c("tab_inferred", "tab_observed")) {
  tmptab <- tempfile()
  TAB.   <- get(tab.)
  colnames(TAB.) <- sprintf("(%i)", 1:ncol(TAB.))
  TAB. <- xtable(TAB., caption = "")
  # paste0(
  #   "Parameter estimates using different estimation methods, priors, and ",
  #   "types of annotations."
  #   ))
  print(TAB., file = tmptab, booktabs = TRUE, scalebox = .6, 
        comment = FALSE, sanitize.text.function= function(i) i)
  TAB. <- readLines(tmptab)
  TAB.[grepl("begin\\{tabular", TAB.)] <- sprintf(
    "\\begin{tabular}{l*{%d}{m{%.2f\\linewidth}}}",
    ncol(theta),
    1.1/ncol(theta)
  )
  TAB.[grepl("Method", TAB.)] <- paste("\\midrule", TAB.[grepl("Method", TAB.)])
  
  tables[[tab.]] <- TAB.
}

```

```{r nice-examples}
# This is without priors MCMC partially annotated
plot(estimates[[3]], which.tree = 104, nsamples = 500)

```


## Estimates (fully annotated trees)

\footnotesize
\begin{minipage}{.47\linewidth}
\centering
Fully annotated trees \\
```{r table-inferred, results='asis'}
cat(tables[["tab_inferred"]])
```
\end{minipage}
\begin{minipage}{.47\linewidth}
\centering
Partially annotated trees \\
```{r table-observed, results='asis'}
cat(tables[["tab_observed"]])
```
\end{minipage}


## MCMC convergence: Fully annotated (Unif prior)

```{r preparing-for-mcmc-diagnostics}
# files <- gsub("(parameter_estimates_)", "", files)
files <- gsub("(mcmc|mle)", "\\U\\1", files, perl = TRUE)
files <- gsub("_", " ", files)
files <- gsub("[.]rds", "", files)
names(estimates) <- files
names(estimates0) <- files

# Getting the 4 smallest trees
inferred <- order(Ntip(estimates0$`MCMC fully annotated no prior`))[1:4]
observed <- order(Ntip(estimates0$`MCMC partially annotated no prior`))[1:4]
```


```{r setting-coda}
library(coda)
traceplot2 <- function(x, ylim = NULL, what = NULL) {
  x <- x$hist
  
  if (!is.null(what))
    x <- what(x)
  
  k <- ifelse(is.mcmc.list(x), ncol(x[[1]]), ncol(x))
  op <- par(mfrow = c(4,2), mai = c(0.3366, 0.2706, 0.2706, 0))
  traceplot(x, ylim = ylim)
  par(op)
}

```


```{r trace-fully-unif}
traceplot2(estimates0$`MCMC fully annotated no prior`)
```

## MCMC convergence: Fully annotated (Beta prior)

```{r trace-fully-beta}
traceplot2(estimates0$`MCMC fully annotated prior`)
```

## MCMC convergence: Partially annotated (Unif prior)

```{r trace-partially-unif}
traceplot2(estimates0$`MCMC partially annotated no prior`)
```

## MCMC convergence: Partially annotated (Beta prior)

```{r trace-partially-beta}
traceplot2(estimates0$`MCMC partially annotated prior`)
```

## How priors affect predictions: Fully annotated (Unif prior)

```{r annotations1.1, cache = TRUE}
plot(
  estimates$`MCMC fully annotated no prior`,
  which.tree = inferred[2],
  cex = .5,
  loo = TRUE,
  nsamples = 1000,
  ncores = 4
  )
```

## How priors affect predictions: Fully annotated (Beta prior)

```{r annotations1.2, cache = TRUE}
plot(
  estimates$`MCMC fully annotated prior`,
  which.tree = inferred[2],
  cex = .5,
  loo = TRUE,
  nsamples = 1000,
  ncores = 4
  )
```


## How priors affect predictions: Experimentally annotated (Unif prior)

```{r annotations2.1, cache = TRUE}
plot(
  estimates$`MCMC partially annotated no prior`,
  which.tree = observed[2],
  cex = .5,
  loo = TRUE,
  nsamples = 1000,
  ncores = 4
  )
```


## How priors affect predictions: Experimentally annotated (Beta prior)

```{r annotations2.2, cache = TRUE}
plot(
  estimates$`MCMC partially annotated prior`,
  which.tree = observed[2],
  cex = .5,
  loo = TRUE,
  nsamples = 1000,
  ncores = 4
  )
```


## Joint on one at a time?

```{r comparing-accuracy, eval=TRUE, echo=FALSE, fig.width=6, fig.height=6, out.width=".7\\linewidth"}

# Disjoint
estimates_disjoint <- readRDS("estimates_disjoint.rds")

aucs_dj <- sapply(estimates_disjoint, function(i) {
  i$ps$auc$auc
})

pss_dj  <- sapply(estimates_disjoint, function(i) {
  with(i$ps, c(obs, random))
})

# Joint
ps_j <- ps[[1]]#[names(estimates) == "MCMC partially annotated no prior"][[1]]
aucs_j <- sapply(ps_j, function(i) {i$auc$auc})
pss_j  <- sapply(ps_j, function(i) {
  with(i, c(obs, random))
})

saveRDS(list(joint = aucs_j, disjoint = aucs_dj),
        file = "aucs.rds")

ttest <- t.test(aucs_j, aucs_dj, paired = TRUE)

dat <- rbind(
  data.frame(AUC = aucs_j, Model = "Pooled"),
  data.frame(AUC = aucs_dj, Model = "One-at-a-time")
)

t.test(
  y = sapply(estimates_disjoint, function(i) i$ps$obs),
  x = pss_j[1,], paired = TRUE, var.equal = FALSE
)

dat <- rbind(
  data.frame(x = pss_j[1,], Model = "Pooled"),
  data.frame(x = sapply(estimates_disjoint, function(i) i$ps$obs), Model = "One-at-a-time")
)

plot(aucs_dj, ans[,1])

ttest <- t.test(
  y = sapply(estimates_disjoint, function(i) i$ps$obs),
  x = pss_j[1,], paired = TRUE, var.equal = FALSE
)

library(ggplot2, quietly = TRUE)
ggplot(dat, aes(y = x, x = Model)) +
  geom_boxplot() + 
  labs(x = "Estimation method", y = "Prediction score") + 
  annotate(
    "text",
    x     = 2,
    y     = .5,
    label = with(
      ttest,
      sprintf("Paired t-test 95%% CI [%.2f, %.2f]", conf.int[1], conf.int[2])
      )
    )
ggsave("comparing-accuracy-1.pdf", width = 5, height = 5, units = "in")
```


---

```{r out-of-sample1, fig.height=9.5}
# http://sifter.berkeley.edu/results-id=6810550
trees <- readRDS("PTHR11409.rds")

aucs <- lapply(trees, function(i) aphylo::auc(i$tip.annotation[,3], i$tip.annotation[,1]))
```

```{r out-of-sample-plot1}
plot(
  trees[[1]],
  as_ci = c(2:4),
  main = sprintf(
    "Adenosine Deaminase (PTHR11409)\nAUCs:={%.2f, %.2f, -}", aucs[[1]]$auc, aucs[[2]]$auc),
  cex = .4)
```

---

```{r out-of-sample-plot2}
plot(
  trees[[2]],
  as_ci = c(2:4),
  main = sprintf(
    "Adenosine Deaminase (PTHR11409)\nAUCs:={%.2f, %.2f, -}", aucs[[1]]$auc, aucs[[2]]$auc),
  cex = .4)
```

---

```{r out-of-sample-plot3}
plot(
  trees[[3]],
  as_ci = c(2:4),
  main = sprintf(
    "Adenosine Deaminase (PTHR11409)\nAUCs:={%.2f, %.2f, -}", aucs[[1]]$auc, aucs[[2]]$auc),
  cex = .4
  )
```
