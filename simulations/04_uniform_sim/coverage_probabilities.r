...
load("simulations/04_uniform_sim/bias.rda")
...
summ <- summ %>% ungroup %>%
  ...
  print %>%
  cat(file = "tables/04_coverage95_by_method_size_missingness.tex")
...
summ %>% ungroup %>%
  ...
  print %>%
  cat(file = "tables/04_coverage95_by_method_missingness.tex")
