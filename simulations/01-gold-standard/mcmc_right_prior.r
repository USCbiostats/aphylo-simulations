...
#SBATCH --job-name=01-mcmc_right_prior
...
job <- Slurm_lapply(
  ...
  job_name   = "01-mcmc_right_prior-lapply",
  plan       = "wait",
  export     = ls()
)

saveRDS(job, paste0(PROJECT_PATH, "/simulations/01-gold-standard/mcmc_right_prior-job.rds"))

saveRDS(
  res <- Slurm_collect(job),
  paste0(
    PROJECT_PATH,
    "/simulations/01-gold-standard/mcmc_right_prior.rds"
    )
  )
