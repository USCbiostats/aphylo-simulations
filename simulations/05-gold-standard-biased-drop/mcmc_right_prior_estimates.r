...
if (file.exists("simulations/05-gold-standard-biased-drop/mcmc_right_prior_estimates.rda")) {
  load("simulations/05-gold-standard-biased-drop/mcmc_right_prior_estimates.rda")
  start <- i - 1L
...
for (i in start:nsim) {
  ...
  if (!(i %% 10)) {
    ...
    save(
      ...
      file     = "simulations/05-gold-standard-biased-drop/mcmc_right_prior_estimates.rda",
      compress = FALSE
      )
