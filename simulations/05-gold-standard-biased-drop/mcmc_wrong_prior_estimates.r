rm(list = ls())
library(aphylo)
load("simulations/05-gold-standard-biased-drop/data_and_functions.rda")

if (file.exists("simulations/05-gold-standard-biased-drop/mcmc_wrong_prior_estimates.rda")) {
  load("simulations/05-gold-standard-biased-drop/mcmc_wrong_prior_estimates.rda")
  start <- i - 1L
...
for (i in 1:nsim) {
  ...
  if (!(i %% 10)) {
    ...
    save(
      ...
      file = "simulations/05-gold-standard-biased-drop/mcmc_wrong_prior_estimates.rda",
      compress = FALSE
      )
