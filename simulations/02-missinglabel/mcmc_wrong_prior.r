...
#SBATCH --job-name=02-mcmc_wrong_prior
...
job <- Slurm_lapply(
  ...
  job_name   = "02-mcmc_wrong_prior-lapply",
  plan       = "wait",
  export     = c("ALPHA_PAR", "BETA_PAR", "ALPHA_PAR_WRONG", "parnames")
)

saveRDS(job, paste0(PROJECT_PATH, "/simulations/02-missinglabel/mcmc_wrong_prior-job.rds"))

saveRDS(
  res <- Slurm_collect(job),
  paste0(
    PROJECT_PATH,
    "/simulations/02-missinglabel/mcmc_wrong_prior.rds"
    )
  )
